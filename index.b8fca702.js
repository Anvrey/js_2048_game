...
let prevState = game.getState().map((row)=>[
        ...row
    ]);
...
function render() {
    ...
    state.forEach((row, r)=>{
        row.forEach((value, c)=>{
            ...
            if (document.body.classList.contains("body__nightmode")) cell.classList.add("field-cell__nightmode");
            if (value !== 0) {
                cell.textContent = value;
                cell.classList.add(`field-cell--${value}`);
                if (document.body.classList.contains("body__nightmode")) cell.classList.add(`field-cell--${value}__nightmode`);
                if (value !== prev) {
...
function restartGame() {
    ...
    button.addEventListener("click", ()=>{
        ...
        game.restart();
        prevState = game.getState().map((row)=>[
                ...row
            ]);
        render();
    });
        ...
        if (body.classList.contains("body__nightmode")) {
            ...
            cells.forEach((cell)=>{
                ...
                cell.classList.forEach((cls)=>{
                    if (/field-cell--\d+__nightmode/.test(cls)) cell.classList.remove(cls);
                });
            });
...
class Game {
    ...
    moveLeft() {
        ...
        this.board = this.board.map((row)=>{
            ...
            for(let i = 0; i < rowCopy.length; i++)if (rowCopy[i] !== newRow[i]) {
                moved = true;
                break;
            }
            return newRow;
    ...
    processRow(row) {
        ...
        for(let i = 0; i < filteredRow.length; i++)if (filteredRow[i] === filteredRow[i + 1]) {
            ...
            this.score += merge;
            i++;
        } else result.push(filteredRow[i]);
    ...
    canMove() {
        for(let r = 0; r < 4; r++)for(let c = 0; c < 4; c++){
            ...
            if (v === 0) return true;
            if (c < 3 && v === this.board[r][c + 1]) return true;
            if (r < 3 && v === this.board[r + 1][c]) return true;
        }
        return false;
    }
}
